...
const AI_DISCLOSURE = "Hi, quick heads up: I'm an AI assistant calling on behalf of an AgentPhone user.";
const AI_DISCLOSURE_SYSTEM = "You are an AI assistant placing an automated phone call on behalf of an AgentPhone user. " +
...
function withDisclosure(greeting) {
 ...
 */
function lockedSystemPrompt(topic) {
    return (`${AI_DISCLOSURE_SYSTEM}\n\n` +
...
export function registerTools(server, api) {
    ...
    // Account Overview
    // ============================================================
    server.tool("account_overview", "Get a complete snapshot of your AgentPhone account: agents, phone numbers, webhook status, " +
        "and usage limits.", {}, { readOnlyHint: true, idempotentHint: true }, async () => {
        try {
            ...
            if (usage) {
                sections.push(`Phone Numbers: ${usage.numbers.used}/${usage.numbers.limit} used (${usage.numbers.remaining} remaining)`);
            }
            else {
            ...
            if (agents) {
                if (agents.data.length > 0) {
                    ...
                    for (const a of agents.data) {
                        const nums = a.numbers && a.numbers.length > 0
                            ? a.numbers.map((n) => n.phoneNumber).join(", ")
                            : "no numbers";
                        sections.push(`  ${a.name} (id=${a.id}, voiceMode=${a.voiceMode}, ${nums})`);
                    }
                }
                else {
                ...
                if (numbers.data.length > 0) {
                    ...
                    for (const n of numbers.data) {
                        const agent = n.agentId
                            ? `agent=${n.agentId}`
                            : "unassigned";
                        sections.push(`  ${n.phoneNumber} (id=${n.id}, ${n.country}, ${n.status}, ${agent})`);
                    }
                }
                else {
            ...
            if (webhook) {
                ...
                sections.push(`  Not configured. Use set_webhook to receive inbound messages and call events.`);
            }
            else {
                sections.push(`  Failed to load.`);
            }
            ...
            if (usage) {
                ...
                sections.push(`  Messages (30d): ${usage.stats.messagesLast30d}  Calls (30d): ${usage.stats.callsLast30d}`);
            }
    ...
    // Phone Numbers
    // ============================================================
    server.tool("list_numbers", "List all phone numbers in your account, each with its ID, status, country, and assigned agent.", {
        limit: z
            .number()
            .min(1)
            .max(100)
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ limit, offset }) => {
        try {
    ...
    server.tool("buy_number", "Purchase a new phone number. This is a paid purchase that charges the account's balance. " +
    ...
    }, { openWorldHint: true }, async ({ country, area_code, agent_id, confirm }) => {
        const countryErr = validateCountry(country);
        if (countryErr)
            return err(new Error(countryErr));
        if (area_code) {
            const areaErr = validateAreaCode(area_code);
            if (areaErr)
                return err(new Error(areaErr));
        }
        if (!confirm) {
            const params = [
                `country="${country}"`,
            ...
            return ok(`This will purchase a new ${country} phone number${area_code ? ` in area code ${area_code}` : ""}` +
        ...
        try {
            ...
            return ok(`Purchased number ${result.phoneNumber} (${result.country})\n\n${JSON.stringify(result, null, 2)}`);
        }
        catch (e) {
            return err(e);
        }
    ...
    // SMS / Messages
    // ============================================================
    server.tool("send_message", "Send an SMS or iMessage from one of your agent's phone numbers, or react to a message.\n\n" +
    ...
    }, { openWorldHint: true }, async ({ agent_id, to_number, body, media_url, media_urls, number_id, from_number, reply_to_message_id, send_style, react_to_message_id, reaction }) => {
        ...
        if (to_number.startsWith("+")) {
            ...
            if (phoneErr)
                return err(new Error(phoneErr));
        }
        if (from_number) {
            const fromErr = validateE164(from_number);
            if (fromErr)
                return err(new Error(fromErr));
        }
        try {
    ...
    server.tool("get_messages", "Get SMS messages for a specific phone number, identified by its number ID. " +
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ number_id, limit }) => {
        try {
    ...
    // Calls
    // ============================================================
    server.tool("list_calls", "List recent calls. Scope by agent_id or number_id, or use status/direction/search to filter globally.\n\n" +
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ agent_id, number_id, limit, offset, status, direction, search }) => {
        try {
            let result;
            if (number_id) {
            ...
            const formatted = result.data
                .map((c) => `[${c.startedAt}] ${c.direction} ${c.fromNumber} → ${c.toNumber} (${c.status}) id=${c.id}`)
                .join("\n");
            return ok(`${result.data.length} call(s):\n\n${formatted}\n\nTotal: ${result.total}\n\nUse get_call with the id= value to fetch transcript.`);
        }
        catch (e) {
            return err(e);
        }
    });
    server.tool("get_call", "Get details and transcript for a specific call, identified by its call ID. " +
        "Pass wait=true to block until an in-progress call finishes before returning.", {
        call_id: z.string().describe("The call ID"),
        wait: z
            .boolean()
            .default(false)
            .describe("When true, long-polls until the call completes before returning. " +
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ call_id, wait, timeout }) => {
        try {
            ...
            if (result.transcripts && result.transcripts.length > 0) {
                transcript = result.transcripts
                    ...
                    const parts = [];
                    if (t.transcript)
                        parts.push(`  Human: ${t.transcript}`);
                    if (t.response)
                        parts.push(`  Agent: ${t.response}`);
                    return parts.join("\n");
                })
                    .filter(Boolean)
                    .join("\n");
    ...
    server.tool("make_call", "Initiate an outbound, webhook-driven phone call where your backend handles the " +
        "conversation logic.\n\n" +
    ...
    }, { openWorldHint: true }, async ({ agent_id, to_number, initial_greeting, from_number_id, voice }) => {
        const phoneErr = validateE164(to_number);
        if (phoneErr)
            return err(new Error(phoneErr));
        try {
            const result = await api.makeCall(agent_id, to_number, withDisclosure(initial_greeting), from_number_id, voice);
    ...
    server.tool("make_conversation_call", "Place a phone call where the AI has an autonomous conversation about a given topic — " +
        ...
        to_number: z
            .string()
            .describe("Recipient phone number in E.164 format (e.g. +14155551234)"),
        topic: z.string().describe("The conversation goal for the assistant to pursue. Embedded as the call objective within " +
        ...
        voice: z
            ...
            .optional()
            .describe("Voice ID override for this call"),
    }, { openWorldHint: true }, async ({ agent_id, to_number, topic, initial_greeting, wait, max_wait_seconds, from_number_id, voice }) => {
        const phoneErr = validateE164(to_number);
        if (phoneErr)
            return err(new Error(phoneErr));
        try {
            const result = await api.makeConversationCall(agent_id, to_number, lockedSystemPrompt(topic), withDisclosure(initial_greeting), wait, max_wait_seconds, from_number_id, voice);
    ...
    // Agents
    // ============================================================
    server.tool("list_agents", "List all agents with their phone numbers and voice configuration. " +
        ...
        limit: z
            ...
            .default(20)
            .describe("Max results to return"),
    }, { readOnlyHint: true, idempotentHint: true }, async ({ limit }) => {
        try {
    ...
    server.tool("create_agent", "Create a new agent. An agent owns phone numbers and handles calls/SMS.\n\n" +
        ...
        model_tier: z
            ...
            .optional()
            .describe("Model quality/speed tier for hosted-mode agents. " +
    ...
    server.tool("update_agent", "Update an agent's configuration — name, description, voice settings, instructions, greeting, " +
        ...
        language: z
            ...
            .optional()
            .describe("BCP-47 locale for speech recognition and synthesis, e.g. 'en-US', 'es-ES', 'ja-JP'."),
    }, { idempotentHint: true }, async ({ agent_id, name, description, voice_mode, system_prompt, begin_message, voice, model_tier, transfer_number, voicemail_message, voice_speed, interruption_sensitivity, enable_backchannel, enable_messaging, stt_mode, ambient_sound, denoising_mode, max_silence_ms, language }) => {
        if (transfer_number) {
            const phoneErr = validateE164(transfer_number);
            if (phoneErr)
                return err(new Error(phoneErr));
        ...
        try {
            ...
            if (voice_mode !== undefined)
                params.voiceMode = voice_mode;
            if (system_prompt !== undefined)
                params.systemPrompt = system_prompt;
            if (begin_message !== undefined)
                params.beginMessage = begin_message;
            if (voice !== undefined)
                params.voice = voice;
            if (model_tier !== undefined)
                params.modelTier = model_tier;
            if (transfer_number !== undefined)
                params.transferNumber = transfer_number;
            if (voicemail_message !== undefined)
                params.voicemailMessage = voicemail_message;
            if (voice_speed !== undefined)
                params.voiceSpeed = voice_speed;
            if (interruption_sensitivity !== undefined)
                params.interruptionSensitivity = interruption_sensitivity;
            if (enable_backchannel !== undefined)
                params.enableBackchannel = enable_backchannel;
            if (enable_messaging !== undefined)
                params.enableMessaging = enable_messaging;
            if (stt_mode !== undefined)
                params.sttMode = stt_mode;
            if (ambient_sound !== undefined)
                params.ambientSound = ambient_sound;
            if (denoising_mode !== undefined)
                params.denoisingMode = denoising_mode;
            if (max_silence_ms !== undefined)
                params.maxSilenceMs = max_silence_ms;
    ...
    server.tool("delete_agent", "Delete an agent permanently. Phone numbers attached to it are kept but unassigned. " +
        "This cannot be undone.", {
        agent_id: z.string().describe("The agent ID to delete"),
    }, { destructiveHint: true }, async ({ agent_id }) => {
        try {
    ...
    server.tool("get_agent", "Get details for a specific agent including its phone numbers, voice configuration, and instructions.", {
        agent_id: z.string().describe("The agent ID"),
    }, { readOnlyHint: true, idempotentHint: true }, async ({ agent_id }) => {
        try {
    ...
    server.tool("attach_number", "Attach a phone number to an agent so the agent handles calls/SMS on that number. " +
        "Takes an agent ID and a phone number ID.", {
        agent_id: z.string().describe("The agent ID"),
        number_id: z.string().describe("The phone number ID to attach"),
    }, { idempotentHint: true }, async ({ agent_id, number_id }) => {
        try {
            const result = await api.attachNumber(agent_id, number_id);
            return ok(`Attached number ${result.number.phoneNumber} to agent ${result.agentId}`);
        }
        catch (e) {
            return err(e);
        }
    });
    server.tool("detach_number", "Detach a phone number from an agent. The number is kept in your account but becomes unassigned.", {
    ...
    server.tool("list_voices", "List available voices for agents. Each voice has a voice_id used in an agent's voice field.", {}, { readOnlyHint: true, idempotentHint: true }, async () => {
        try {
            ...
            const formatted = result.data
                .map((v) => `${v.voice_id} — ${v.voice_name} (${v.provider}, ${v.gender}, ${v.accent})`)
                .join("\n");
            return ok(`${result.data.length} voice(s) available:\n\n${formatted}`);
        }
        catch (e) {
            return err(e);
        }
    ...
    // Conversations
    // ============================================================
    server.tool("list_conversations", "List SMS conversations. Optionally filter by agent_id to see conversations for a specific agent.\n\n" +
        ...
        offset: z
            ...
            .default(0)
            .describe("Number of results to skip (for pagination)"),
    }, { readOnlyHint: true, idempotentHint: true }, async ({ agent_id, limit, offset }) => {
        try {
            ...
            const formatted = result.data
                ...
                .join("\n");
            return ok(`${result.data.length} conversation(s):\n\n${formatted}\n\nTotal: ${result.total}`);
        }
        catch (e) {
            return err(e);
        }
    });
    server.tool("get_conversation", "Get a specific SMS conversation with full message history, identified by its conversation ID.", {
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ conversation_id, message_limit }) => {
        try {
    ...
    server.tool("update_conversation", "Set metadata on a conversation. Use this to store custom state, tags, or context " +
    ...
    }, { idempotentHint: true }, async ({ conversation_id, metadata }) => {
        try {
    ...
    // Contacts
    // ============================================================
    server.tool("list_contacts", "List saved contacts (your address book). Optionally filter with a search term that matches " +
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ search, limit, offset }) => {
        try {
    ...
    server.tool("manage_contact", "Create, update, or delete a saved contact. Set `action` to choose the operation.\n\n" +
    ...
    }, { destructiveHint: true }, async ({ action, contact_id, phone_number, name, email, notes }) => {
        if (phone_number) {
        ...
        try {
            ...
            if (action === "update") {
                if (!contact_id)
                    return err(new Error("update requires contact_id."));
                const params = {};
            ...
            // action === "delete"
            if (!contact_id)
                return err(new Error("delete requires contact_id."));
    ...
    // Usage
    // ============================================================
    server.tool("get_usage", "Get account usage statistics. By default returns a summary with number usage plus " +
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ breakdown, days, months }) => {
        try {
            if (breakdown === "daily") {
                ...
                const formatted = result.data
                    .map((d) => `${d.date}: ${d.messages} msgs, ${d.calls} calls, ${d.webhooks} webhooks`)
                    .join("\n");
            ...
            if (breakdown === "monthly") {
                ...
                const formatted = result.data
                    .map((d) => `${d.month}: ${d.messages} msgs, ${d.calls} calls, ${d.webhooks} webhooks`)
                    .join("\n");
    ...
    // ============================================================
    server.tool("get_webhook", "Get the webhook configuration. Pass agent_id to get an agent-specific webhook, " +
    ...
    server.tool("set_webhook", "Set a webhook URL to receive inbound messages and call events.\n\n" +
    ...
    }, { idempotentHint: true }, async ({ url, agent_id, context_limit, timeout }) => {
        try {
            const result = agent_id
            ...
            return ok(`${scope} webhook set!\n  URL: ${result.url}\n  Secret: ${result.secret}\n  Status: ${result.status}`);
        }
        catch (e) {
            return err(e);
        }
    });
    server.tool("delete_webhook", "Remove a webhook. Pass agent_id to remove an agent's webhook (falls back to project default). " +
    ...
    server.tool("test_webhook", "Send a test event to verify a webhook is working. Returns the HTTP status code and response time.\n\n" +
    ...
    }, { idempotentHint: true, openWorldHint: true }, async ({ agent_id }) => {
        try {
            const result = agent_id
            ...
            if (result.success) {
                return ok(`${scope} test successful!\n  Status code: ${result.statusCode}\n  Response time: ${result.responseMs}ms`);
            }
            return ok(`${scope} test failed.\n  Error: ${result.error}\n  Status code: ${result.statusCode}`);
        }
        catch (e) {
            return err(e);
        }
    });
    server.tool("list_webhook_deliveries", "View recent webhook delivery history. Shows which events were delivered, HTTP status codes, and timing.\n\n" +
    ...
    }, { readOnlyHint: true, idempotentHint: true }, async ({ agent_id, limit, hours }) => {
        try {
            const result = agent_id
            ...
            const formatted = result.data
                .map((d) => `[${d.deliveredAt}] ${d.event} → ${d.success ? "OK" : "FAILED"} (${d.statusCode ?? "N/A"}, ${d.responseMs ?? "N/A"}ms) id=${d.id}`)
                .join("\n");
            return ok(`${result.data.length} delivery(ies):\n\n${formatted}\n\nTotal: ${result.total}`);
        }
        catch (e) {
            return err(e);
        }
